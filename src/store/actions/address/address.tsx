...
import { SetLocationsSearchsLoadingAction, AddressModel, AddAddressAction, EditAddressAction, SetChoseAddressModeAction, LocationModel, SetCurrentLocationAction, SetCurrentLocationNotFoundAction, SetLocationsSearchsAction, SetLocationsSearchsNotFoundAction, SetSearchsLocationsTermAction, SetAddressesAction, RemoveAddressAction } from "./addressTypes";
...
export function setCurrentLocation(currentLocation: LocationModel): SetCurrentLocationAction {
  return {
...
export function setLocationSearchs(locations: LocationModel[]): SetLocationsSearchsAction {
  return {
...
export function setSearchLocationTerm(searchLocationsTerm: string): SetSearchsLocationsTermAction {
  return {
...
export function handleSetCurrentLocation(key: string, location?: LocationModel) {
...
export function handleSearchLocationByTerm(search: string, cep: string) {
  return async (dispatch: any) => {
    ...
    dispatch(setLocationSearchsLoading());
      ...
      setTimeout(async () => {
        if (serachLocationId === currentId && search !== "" && search.length === 9) {
          try {
