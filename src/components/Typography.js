...
export const H1 = styled.Text`
  ...
  color: ${({ type, theme }) => type ? theme.colors[type] : theme.colors.darkColor};
`;
...
export const Subtitle1 = styled.Text`
  ...
  color: ${({ type, color, theme }) => color ? color : type ? theme.colors[type] : theme.colors.darkColor};
  ${({ center }) => center ? "text-align: center;" : ""}
  ${({ underline, color }) => underline ? `text-decoration: underline; text-decoration-color: ${color ? color : "inherit"};` : ""};
`;
